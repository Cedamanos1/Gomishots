...
// Función para movimientos por mes (compatible con PostgreSQL y MySQL)
function movimientosPorMes($pdo, $fecha_inicio, $fecha_fin) {
    ...
    try {
        ...
        if ($driver === 'pgsql') {
            ...
            $date_format = "TO_CHAR(fecha, 'YYYY-MM')";
        } else {
            // MySQL usa DATE_FORMAT
            $date_format = "DATE_FORMAT(fecha, '%Y-%m')";
        }
        
        // Ingresos por mes
        $stmt_ing = $pdo->prepare("SELECT {$date_format} as mes, SUM(cantidad) as total 
                                   FROM ingresos 
        ...
        // Salidas por mes
        $stmt_sal = $pdo->prepare("SELECT {$date_format} as mes, SUM(cantidad) as total 
                                   FROM salidas 
                                   WHERE fecha BETWEEN :inicio AND :fin 
                                   GROUP BY mes 
                                   ORDER BY mes");
